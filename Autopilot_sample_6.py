...
def draw_hud(img, fps, r, b, g, stage):
    ...
    names = {1:"FWD→青待ち", 2:"LEFT→緑待ち", 3:"CCW180(非同期)", 4:"FWD→青待ち(2)", 5:"LEFT→赤待ち"}
    cmdline = f"CMD: {'BUSY ' + cmd_name if cmd_busy else 'IDLE'}"
    info = [
    ...
    while not finish:
        ...
        if stage == 1:
            ...
            if blue_per >= BLUE_THR:
                ...
                stage = 2
            elif timer_flag and not cmd_busy:
            ...
            # 左旋回180°を非同期で発行→完了まで待つ
            if not cmd_busy and not cmd_done_evt.is_set():
                ok = send_async("ccw", YAW_CCW)
                if ok:
                    log_key(f"[STAGE] 発行: ccw {YAW_CCW} （完了待ち）")
            # 完了したら次へ
            if cmd_done_evt.is_set() and not cmd_busy:

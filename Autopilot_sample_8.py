...
try:
    ...
    time.sleep(0.1)  # ちょい待つと安定しやすい
    try:
        bat = tello.Send("battery?")
        # tello.Send が文字列を返す実装ならここで表示できる
        if bat is not None:
            log_key(f"[BATTERY] {bat}%")
        else:
            log_key("[BATTERY] battery? sent (no return)")
    except Exception as e:
        log_err(f"battery? failed: {e!r}")
    ...
    while not finish:
        ...
        if rotating_now and cmd_done_evt.is_set() and (not cmd_busy):
            rotating_now = False
            rotated_done = True
            log_key("[ACTION] ccw 完了！(done=True)")
